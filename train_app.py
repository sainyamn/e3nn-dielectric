    ...
    try:
        ...
        def create_wrapped_model(original_model_fn, params):
            ...
            def wrapped_model(vectors, species, senders, receivers):
                ...
                # Debug: report shapes
                metrics_queue.put({"status": f"Model output type: {type(output)}, length: {len(output) if isinstance(output, tuple) else 'not tuple'}", "type": "info"})
                ...
                if isinstance(output, tuple):
                    if len(output) >= 3:
                        ...
                        # Debug shapes
                        metrics_queue.put({"status": f"BECS shape: {node_becs.shape}, EPS shape: {node_eps.shape}", "type": "info"})
                        
                        # Fix epsilon shape if needed
                        # EPS should be per-node initially, then aggregated to per-graph in predict_becs_eps
                        # If eps has wrong shape, fix it
                        if len(node_eps.shape) != 3 or node_eps.shape[-1] != 3 or node_eps.shape[-2] != 3:
                            # If eps has wrong shape, create a proper tensor
                            metrics_queue.put({"status": f"Fixing EPS shape from {node_eps.shape} to match BECS", "type": "info"})
